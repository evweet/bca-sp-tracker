...
for database in "${DATABASES[@]}"; do
    ...
    OUTPUT_DIR="${REPO_DIR}/${OUTPUT_SUBDIR}/${database}"
    mkdir -p "$OUTPUT_DIR"
    ...
    echo "Extracting from database: ${database}"

    for sp in "${PROCEDURES[@]}"; do
        sp="$(trim_whitespace "$sp")"
        [[ -z "$sp" ]] && continue
...
ORDER BY p.oid")"
        ...
        set -e
        if [[ $rc -ne 0 ]]; then
            echo "ERROR: Failed to list overloads for ${database}.${SCHEMA}.${sp}" >&2
            FAILURES=$((FAILURES+1))
            continue
        fi

        if [[ -z "${list_out//[[:space:]]/}" ]]; then
            echo "WARNING: Not found in ${database}: ${SCHEMA}.${sp}" >&2
            continue
        fi
        ...
        for row in "${rows[@]}"; do
            ...
            set -e
            if [[ $rc -ne 0 ]]; then
                echo "ERROR: pg_get_functiondef failed for ${database} oid ${oid}" >&2
                FAILURES=$((FAILURES+1))
                continue
            fi
...
"
            ...
            echo "Extracted: ${database}/${file_name}"
        done
    done
done
